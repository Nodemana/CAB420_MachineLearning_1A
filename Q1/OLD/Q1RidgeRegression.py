...
Y_train, Ytrain_mean, Ytrain_stdev = standardise(Y_train)
X_val, Xval_mean, Xval_stdev = standardise(X_val)
y_val, yval_mean, yval_stdev = standardise(y_val)
X_test, Xtest_mean, Xtest_stdev = standardise(X_test)
y_test, ytest_mean, ytest_stdev = standardise(y_test)
print(X_train)

#Create model with Lambda = 1
str = "ViolentCrimesPerPop ~" + "+".join(X_train.columns)
model = sm.ols(formula=str, data=train).fit_regularized(alpha=0.5, L1_wt=0)
